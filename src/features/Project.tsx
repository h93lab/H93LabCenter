...
export default function Project() {
  ...
  const [tab, setTab] = useState("Overview"),
    ...
    [docPath, setDocPath] = useState("README.md"),
              ...
              <div className="two-column">
                ...
                <section className="card">
                  ...
                  <div className="prose-card">
                    <p>
                      {b.requirements.length} requirements · {b.screens.length}{" "}
                      screens · {b.tasks.length} tasks · {b.documents.length}{" "}
                      documents
                    </p>
                  </div>
                </section>
                ...
                d.changes.map((c: Row) => (
                  <article className="change-card" key={c.id}>
                    ...
                    {c.impact_summary?.impacts && (
                      <>
                        <p>
                          {c.impact_summary.impacts.length} artifacts affected.
                          Applying creates a new draft and regenerates the
                          prototype.
                        </p>
                        <div className="table-wrap">
                          <table>
          ...
          {tab === "Versions" && (
            <section className="card">
              ...
              {d.versions.map((version: Row, i: number) => {
                ...
                const changed = version.manifest.documents.filter(
                  (f: Row) =>
                    previous?.manifest.documents.find(
                      (old: Row) => old.path === f.path,
                    ...
                    <div className="actions">
                      ...
                      {previous && (
                        <button
                          ...
                          onClick={() =>
                            setEntity({
                              from: previous.version_number,
                              to: version.version_number,
                              documents: changed.map((f: Row) => ({
                                path: f.path,
                                before: previous.manifest.documents.find(
                                  (old: Row) => old.path === f.path,
                                )?.content_md,
                                after: f.content_md,
                              })),
